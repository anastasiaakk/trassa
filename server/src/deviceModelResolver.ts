...
import type { DeviceLabelExtras } from "./deviceLabelDetail.js";
import {
  formatMarketingDeviceName,
  parseAndroidModelFromUa,
  resolveFromModelHint,
} from "./deviceModelNames.js";
import { resolveAppleDeviceModel } from "./iphoneModels.js";
import { resolveAppleFromGpuFingerprint } from "./appleGpuFingerprint.js";
...
export function collectModelCandidates(input: ModelResolveInput): ResolvedDeviceModel[] {
  ...
  if (!/Android/i.test(ua)) {
    ...
    const apple = resolveAppleDeviceModel(
      ua,
      extras?.screenW,
      extras?.screenH,
      extras?.devicePixelRatio
    );
    if (apple) {
...
export function pickBetterStoredModel(
  ...
  if (!incoming) {
    return stored.name
      ? { name: stored.name, confidence: stored.confidence, source: "stored" }
      : null;
  }
