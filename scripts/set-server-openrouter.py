#!/usr/bin/env python3
"""OpenRouter API key on VPS — бесплатные мощные модели (120B, 70B)."""
...
PROVIDER_LINE = "AI_PROVIDER=openrouter"
MODEL_LINE = "OPENROUTER_MODEL=openai/gpt-oss-120b:free"
...
def main() -> int:
    api_key = os.environ.get("TRASSA_OPENROUTER_API_KEY", "").strip() or os.environ.get(
        "OPENROUTER_API_KEY", ""
    ).strip()
    model = os.environ.get("TRASSA_OPENROUTER_MODEL", "").strip() or os.environ.get(
        "OPENROUTER_MODEL", "openai/gpt-oss-120b:free"
    ).strip()
    if not api_key:
        print("Set TRASSA_OPENROUTER_API_KEY, then rerun.", file=sys.stderr)
        return 1
    ...
    out = _append_if_missing(out, "OPENROUTER_MODEL", f"OPENROUTER_MODEL={model}")
