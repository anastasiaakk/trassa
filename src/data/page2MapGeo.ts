...
const SUBJECT_LABEL_OVERRIDES = new Map<string, string>([
  ["Москва", "Москва"],
...
export function formatSubjectDisplayName(name: string): string {
  const n = name.trim();
  if (!n) return "";
  const override = SUBJECT_LABEL_OVERRIDES.get(n);
  if (override) return override;
  if (n.endsWith("ская")) return `${n} область`;
  if (n.endsWith("ский")) return `${n} край`;
  if (n.includes("Осетия") || n.includes("Черкесская") || n.includes("Балкарская")) {
    return `Республика ${n}`;
  }
  return n;
}
