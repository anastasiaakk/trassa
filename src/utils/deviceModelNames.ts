/**
 * Маркетинговые названия устройств (POCO X3 Pro, iPhone 15 Pro, Galaxy S23…).
 * Синхронизировать с server/src/deviceModelNames.ts
 */
...
/** Уже человекочитаемое имя (POCO X3 Pro, iPhone 15 Pro). */
function looksLikeMarketingName(s: string): boolean {
  ...
  if (/\s/.test(t) && /[a-zA-Zа-яА-Я]/.test(t) && !CODENAME_LIKE.test(t.replace(/\s/g, ""))) {
    return true;
  }
  return false;
}
...
/** Модель из User-Agent (сегмент перед Build/). */
export function parseAndroidModelFromUa(ua: string): string | null {
  ...
  if (segments) {
    ...
    for (let i = parts.length - 1; i >= 0; i--) {
      ...
      if (CODENAME_LIKE.test(candidate)) {
        const m2 = lookupAndroidCodename(candidate);
        if (m2) return m2;
      }
    }
  }
